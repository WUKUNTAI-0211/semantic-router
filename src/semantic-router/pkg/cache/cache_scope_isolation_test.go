...
func TestSearchEnforcesUserScope(t *testing.T) {
	...
	for _, useHNSW := range []bool{false, true} {
		...
		t.Run(path, func(t *testing.T) {
			...
			search := c.runFindSimilarEmbeddingSearch(emb, bobQ, 0.8, bobScope)
			...
			search = c.runFindSimilarEmbeddingSearch(emb, aliceQ, 0.8, aliceScope)
			...
			search = c.runFindSimilarEmbeddingSearch(emb, base, 0.8, "")
			if search.bestIndex != -1 {
